...
export async function POST(request: NextRequest) {
  ...
  // Wrapped in try-catch so Arcjet timeouts don't block generation (fail-open).
  ...
  try {
    ...
    console.warn("[gen-ai-code] Arcjet check failed (allowing request):", arcjetErr);
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (!accumulated.trim()) {
          console.error("[gen-ai-code] Gemini returned empty response");
          enqueue(
            sseEvent("error", {
        ...
        try {
          ...
          // Gemini sometimes wraps JSON in markdown fences or adds extra text.
          // Try to extract a valid JSON object from the accumulated output.
          try {
            ...
            // If that didn't help, try to find the outermost { ... }
            ...
            console.error(
              "[gen-ai-code] Failed to parse AI response. Raw output:",
        ...
        if (
          ...
          message = "Gemini API rate limit exceeded (429). Please wait for the daily reset or upgrade your AI Studio tier.";
        } else if (
          err?.status === 503 ||
          ...
          message = "Gemini API is currently experiencing high demand. Please try again in a few seconds.";
        }
        enqueue(
          sseEvent("error", {
            message,
