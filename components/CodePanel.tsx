  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 overflow-hidden h-full">
        ...
        {/* Subtle improving indicator — doesn't block code visibility */}
        {isImproving && (
          <div className="absolute inset-x-0 top-0 z-20 flex items-center gap-2 border-b border-violet-500/20 bg-violet-950/60 backdrop-blur-sm px-4 py-2">
            <Loader2 className="h-3.5 w-3.5 animate-spin text-violet-400" />
            <span className="text-xs font-medium text-violet-300/80">
              Agent is updating files…
            </span>
          </div>
        )}
...
export function CodePanel({
  ...
  // Auto-switch to code tab when improving starts so users see live file updates
  useEffect(() => {
    if (isImproving) setActiveTab("code");
  }, [isImproving]);

  // Auto-switch to preview when fileData arrives (but not during improve —
  // we want to stay on the code tab while patches stream in)
  useEffect(() => {
    if (fileData && !isImproving) setActiveTab("preview");
  }, [fileData, isImproving]);
