        ...
        while (true) {
          ...
          for (const line of lines) {
            ...
            if (event.type === "thinking") {
              ...
              // Apply patch to fileData state immediately so Code tab
              // shows live updates as each file is written by the agent.
              // SandpackProvider key is based on file paths only, so
              // content-only changes go through sandpack.updateFile()
              // without remounting.
              setFileData((prev) => {
                if (!prev) return prev;
                return {
              ...
              // Apply final complete fileData from server
              setFileData(event.fileData);
              setCredits(event.creditsRemaining);
